...
export const WorkHistory = () => {
    ...
    useEffect(() => {
        const getExperienceData = async () => {
          try {
            ...
            const querys = query(experienceRef, where('Jobseeker', '==', auth.currentUser?.uid));
    ...
    const handleAddExperience = () => {
      ...
      const handleExperienceChange = (index: number, key: string, value: string | Timestamp | string[]) => {
  ...
  const handleSubmitExperience = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      for (const experienceEntry of workExperience) {
        const { company_name, job_title, start_date, end_date } = experienceEntry; // Destructure experience properties
  
        // Build a query to check for existing experience with same company & job title within date range
        const existingExpQuery = query(experienceCollection, 
        ...
        if (querySnapshot.empty) {
          ...
          console.log(`Experience for ${company_name} - ${job_title} already exists within the provided date range.`);
...
<div className="experience-section">
        {workExperiences.length > 0 ? (
          workExperiences.map((experience, index) => (
            ...
            <p>{experience.start_date instanceof Timestamp ? experience.start_date.toDate().toISOString() : experience.start_date || ''}</p>
            <p>{experience.sector}</p>
            <p>{experience.end_date instanceof Timestamp ? experience.end_date.toDate().toISOString() : experience.end_date || 'Current Job'}</p>
            <hr></hr>
            </div>
    ...
    <div
          ...
          workExperience.map((experience, index) => (
            ...
            />
              ...
              <button onClick={handleAddExperience}>Add New Work Experience</button>
              <br></br>

                {workExperience.length>0&&<button type="submit">Submit Experience</button>}
            </div>
...
interface ExperienceFormProps {
  ...
  const ExperienceForm: React.FC<ExperienceFormProps> = ({ index, experience, handleChange, onDelete }) => {
    // Define options for select fields
    const sectors = [ // Replace with your actual sectors
  ...
  // ... other sectors
    ...
    return (
      ...
      <input type="text" style={{
                  ...
                  fontSize: "14px",
                  fontWeight: "bold",
                }} id="company_name" value={experience.company_name} onChange={(e) => handleChange(index, 'company_name', e.target.value)} required />
      ...
      <input type="text" style={{
                  ...
                  fontSize: "14px",
                  fontWeight: "bold",
                }} id="job_title" value={experience.job_title} onChange={(e) => handleChange(index, 'job_title', e.target.value)} required />
      ...
      <input type="date" style={{
                  ...
                  fontSize: "14px",
                  fontWeight: "bold",
                }} id="start_date" value={experience.start_date instanceof Timestamp ? experience.start_date.toDate().toISOString() : experience.start_date || ''} onChange={(e) =>handleChange(index, 'start_date', e.target.value)} />
      ...
      <input type="date" style={{
                  ...
                  fontSize: "14px",
                  fontWeight: "bold",
                }} id="end_date" value={experience.end_date instanceof Timestamp ? experience.end_date.toDate().toISOString() : experience.end_date || ''} onChange={(e) => handleChange(index, 'end_date', e.target.value)} />
      ...
      <select id="sector" style={{
                  color: "#1A4F6E",
                  fontSize: "14px",
                  fontWeight: "bold",
                }} value={experience.sector} onChange={(e) => handleChange(index, 'sector', e.target.value)} required>
        {sectors.map((option) => (
      ...
      <textarea id="description" style={{
                  ...
                  fontSize: "14px",
                  fontWeight: "bold",
                }} value={experience.description} onChange={(e) => handleChange(index, 'description', e.target.value)} />
